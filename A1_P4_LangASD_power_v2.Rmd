...
fixed(simple_model)['Diagnosis']<-0.3
...
sim1 = createNewData(100, 6, simple_model)
...
fixef(fav_model)['Visit'] <- -0.02
fixed(fav_model)['Diagnosis']<-0.03
powerCurveV = powerCurve(fav_model, fixed('Visit'), along= 'subj_id', nsim = 10)

plot(powerCurveV)


#100 participants seems to be a fair amount, reaching exactly 80 % although with sligthly large errorbars.
...
#how do i do thiiiiiis
td15 = td %>%
  group_by(as.factor(subj_id = 1, 3, 8, 9, 10, 11, 12, 13, 14, 15, 16, 18, 22, 25, 27)) %>%
  summarise(visit = n())
